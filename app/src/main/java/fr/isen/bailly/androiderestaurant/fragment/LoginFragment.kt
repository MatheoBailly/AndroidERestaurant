...
import fr.isen.bailly.androiderestaurant.databinding.FragmentLoginBinding
    ...
    private fun Login() {
        ...
        if (errorBool) {
            ...
            val request = JsonObjectRequest(
                ...
                { response ->
                    val login = GsonBuilder().create().fromJson(response.toString(), SignupModel::class.java)
                    val editor = requireContext().getSharedPreferences(DetailActivity.APP_PREFS, Context.MODE_PRIVATE).edit()
                    editor.putString(USER_ID, login.data.userId)
                    editor.apply()
